...
- [ ] Students can inspect manually assigned 2D meaning vectors, Euclidean distances, and semantic ranking.
...
- [ ] The interface is readable on classroom projectors, works on desktop and reasonably on tablets, and does not rely on color alone.
...
- Cosine similarity, draggable or editable vectors, challenge mode, presentation mode, scenario import/export, quizzes, and Indonesian scenarios - deferred until after the MVP.
- Mobile-first optimization - MVP targets desktop projectors and tablets.
...
- The canonical flow is query and documents, tokenization, simple matching, TF, IDF, TF-IDF, keyword ranking, keyword limitation, meaning vectors, Euclidean distance, and final comparison.
...
- **Search model**: Simplified TF-IDF and Euclidean distance over manually assigned 2D vectors - calculations must be understandable and reproducible by students.
- **Interaction**: Desktop-first three-panel experience with reasonable tablet support - classroom projection is the primary environment.
